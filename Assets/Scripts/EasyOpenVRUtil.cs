/**
 * EasyOpenVRUtil by gpsnmeajp v0.05(+edit)
...
namespace EasyLazyLibrary
{
    public class EasyOpenVRUtil
    {
        ...
        public string GetDeviceDebugInfoFormatted(uint idx)
        {
            string s = "Device ID:" + idx + " ";
            if (!IsDeviceConnected(idx))
            {
            ...
            */
            ...
            if (!float.IsNaN(batt))
            {
                s += "\n  DeviceBattery: " + ((int)(batt + 0.5f)).ToString() + "% ";
            }
            else
            {
                s += "\n  DeviceBattery: ---% ";
        ...
        public string PutDeviceInfoListStringFormatted()
        {
            string s = "";
            int connectedDeviceNum = ConnectedDevices();
            //deviceの詳細情報を1つづつ読み出す
            ...
            for (uint i = 0; i < OpenVR.k_unMaxTrackedDeviceCount; i++)
            {
                ...
                if (IsDeviceConnected(i))
                {
                    s += GetDeviceDebugInfoFormatted(i) + "\n";
                    connectedDeviceCount++;
                }
                //接続されている数だけ読み取り終わったら終了する
                if (connectedDeviceCount >= connectedDeviceNum)
